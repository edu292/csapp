...
def main():
    ...
    p = subprocess.Popen("./test-csim", shell=True, stdout=subprocess.PIPE, text=True)
    ...
    trans32_score = compute_miss_score(miss32, 300, 600, maxscore["trans32"]) * int(
        result32[0]
    )
    trans64_score = compute_miss_score(miss64, 1300, 2000, maxscore["trans64"]) * int(
        result64[0]
    )
    trans61_score = compute_miss_score(miss61, 2000, 3000, maxscore["trans61"]) * int(
        result61[0]
    )
